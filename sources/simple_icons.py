...
class SimpleWindow(BasicWindow):
    name = "simple_window"
...
class SimpleIcon(RemoteFile):
    def __init__(self, source, info):
        super().__init__(source, info)
        self.name = f"{self.info['name']}-simple"
...
class SimpleIconsPage(RemotePage):
...
class SimpleIconsSource(SvgSource):
    ...
    def search(self, query):
        ...
        self.results = [(key.split(".")[0], base64.b64decode(value)) for key, value in self.icon_map.items()
